...
contract Constants {
  ...
  uint32 internal constant HUB_SET_LIST_ENTRY_BITS_MO_X   =  9; // Hub.SetListEntryBitsMO()
  ...
  uint32 internal constant LE_P_REFUND_S_CAP_MISS_B   =   32768; // 15 Pfund funds Refund due to soft cap not being reached
  uint32 internal constant LE_P_REFUND_SALE_CLOSE_B   =   65536; // 16 Pfund funds Refund due to not being whitelisted by the time that the sale closes
  uint32 internal constant LE_P_REFUND_ONCE_OFF_B     =  131072; // 17 Pfund funds Refund once off manually for whatever reason
  uint32 internal constant LE_MNP_REFUND_S_CAP_MISS_B =  262144; // 18 Mfund but not presale Refund due to soft cap not being reached
  uint32 internal constant LE_M_REFUND_TERMINATION_B  =  524288; // 19 Mfund or Presale with picos Refund proportionately according to Picos held following a vote for project termination
  uint32 internal constant LE_M_REFUND_ONCE_OFF_B     = 1048576; // 20 Mfund funds Refunded once off manually for whatever reason including downgrade from whitelisted
  // Combos
  ...
  uint32 internal constant LE_REFUNDED_COMBO_B         = 2064384; // LE_P_REFUND_S_CAP_MISS_B | LE_P_REFUND_SALE_CLOSE_B | LE_P_REFUND_ONCE_OFF_B | LE_MNP_REFUND_S_CAP_MISS_B | LE_M_REFUND_TERMINATION_B | LE_M_REFUND_ONCE_OFF_B
  ...
  uint32 internal constant LE_NO_REFUND_COMBO_B        = 2080770; // LE_DEAD_COMBO_B | LE_SALE_CONTRACT_B Starting point check. Could also be more i.e. no funds or no PIOs
