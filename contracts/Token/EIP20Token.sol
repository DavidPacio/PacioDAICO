...
EIP20 Methods
=============
function balanceOf(address accountA) public view returns (uint256 balance);
function transfer(address toA, uint256 value) public returns (bool success);
function transferFrom(address frA, address toA, uint256 value) public returns (bool success);
function approve(address spenderA, uint256 value) public returns (bool success);
function allowance(address accountA, address spenderA) public view returns (uint256 remaining);
...
contract EIP20Token is Owned {
  ...
  I_ListToken internal iListC; // the list contract. Set by Token.Initialise() or updated by Token.NewListContract() via Sale.NewListContract()
...
//mapping(address => mapping (address => uint256)) public allowed; - Public in Consensys code, but this doesn't need to be public. private used instead.
  ...
  function transfer(address toA, uint256 value) public IsTransferOK(msg.sender, toA) returns (bool success) {
  ...
  function transferFrom(address frA, address toA, uint256 value) public IsTransferOK(frA, toA) returns (bool success) {
